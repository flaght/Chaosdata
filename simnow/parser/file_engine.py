...
class FileUnit(object):
    ...
    def set_unit(self, dir, filename):
        #/kywk/fin/futures/simnow/20170818/jm1802
        self.__symbol = os.path.split(filename)[-1]
...
class FileIO(object):
    ...
    def write_date_depthmarket(self, exchange_id, contract_object, symbol, root, name, content): #写入日数据
        #/root/XDCE/FB/FB11001/2017-08/1.txt
        dirname = self.__base_path + "/" + exchange_id + "/" + contract_object + "/" + symbol + "/" + root
...
class FileEngine(object):
    ...
    def fetch_files(self):
        for root, _, filenames in file_io.walk(self.__path):
            for f in fnmatch.filter(filenames, '*'):
